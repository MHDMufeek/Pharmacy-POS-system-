...
>>>>>>> 296b82e6ae09dd8820fce6d2be42e34e86375736
    ...
    <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200 dark:bg-slate-800 dark:border-slate-700">
      ...
      <table class="w-full border-collapse min-w-[720px]">
        <thead>
          <tr class="bg-gray-100 text-left text-sm text-gray-700 uppercase tracking-wider dark:bg-slate-800 dark:text-gray-300">
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="min-w-0">
                  <div class="flex items-center gap-3">
                    <p class="font-medium text-gray-900 truncate dark:text-white" :title="expense.name">{{ expense.name }}</p>
                    <button v-if="expense.description" @click.prevent.stop="toggleExpand(getExpenseId(expense, index))" class="text-xs text-blue-600 hover:underline description-toggle">
                      {{ expandedHas(getExpenseId(expense, index)) ? 'Hide' : 'Description' }}
                    </button>
                  </div>
                  <p v-if="expandedHas(getExpenseId(expense, index))" class="text-sm text-gray-700 mt-1 break-words expense-description dark:text-gray-200">{{ expense.description }}</p>
                </div>
              </div>
            </td>
            <td class="p-3 border-b">
