<template>
  <div class="p-6">
    ...
    <!-- Bar Chart Section -->
    <div class="bg-white p-4 rounded-lg shadow mb-6">
      <h2 class="text-lg font-bold text-gray-700 mb-4">Movement Overview</h2>
    ...
    <div class="flex justify-between items-center mt-6 bg-white p-4 rounded-lg shadow">
      ...
      <div class="flex space-x-2">
        ...
        <button class="px-3 py-1 rounded-md bg-blue-600 text-white hover:bg-blue-700">1</button>
        ...
        <button class="px-3 py-1 rounded-md bg-gray-100 text-gray-700 hover:bg-gray-200">Next</button>
      </div>
    </div>
  </div>
</template>
...
// Chart Data - Totals by movement type
const chartData = computed(() => {
  let incoming = 0, outgoing = 0, adjustments = 0
  ...
  return {
    labels: ['Incoming', 'Outgoing', 'Adjustments'],
    datasets: [
      {
        label: 'Drug Movements',
        data: [incoming, outgoing, adjustments],
...
const chartOptions = {
  ...
  plugins: {
    legend: { position: 'top' },
    title: { display: true, text: 'Drug Movement Summary' },
  },
}
